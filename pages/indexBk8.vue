                      ...
                      <div v-if="user[symptom.key] === 'yes'">
                        ...
                        <template v-if="symptom.key === 'hypertensionPoorControl'">
                          ...
                          <v-text-field
                            ...
                            clearable
                            :rules="[
                              (v) => (user.hypertensionPoorControl === 'yes' 
                                      && user.hypertensionMedicationAdherence !== 'none')
                                    ? (v ? true : '必填')
                                    : true
                            ]"
                          />
...
const user = ref<any>({
  ...
  // 先建立所有群組的通用欄位（hypertensionPoorControl 之後用自訂欄位替代顯示）
...
/* 目前血壓用藥為「無」時，自動清空藥名；若關閉整個群組也一併清空 */
watch(() => user.value.hypertensionMedicationAdherence, (val) => {
  if (val === 'none') user.value.hypertensionMedNames = ''
})
watch(() => user.value.hypertensionPoorControl, (val) => {
  if (val !== 'yes') {
    user.value.hypertensionRecentSys = null
    user.value.hypertensionRecentDia = null
    user.value.hypertensionMedicationAdherence = null
    user.value.hypertensionMedNames = ''
  }
})
