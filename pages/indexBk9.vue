            ...
            <v-card elevation="2" title="基本資料與病史">
              <v-card-text>
                <v-form @submit.prevent="submitForm">
                  ...
                  <v-row>
                    <v-col cols="12" sm="6">
                      <v-text-field
                        ...
                        suffix="mmHg"
                        variant="outlined"
                        :rules="[intRange(60,260,'請輸入60–260')]"
                      />
                    </v-col>
                    <v-col cols="12" sm="6">
                      <v-text-field
                        ...
                        suffix="mmHg"
                        variant="outlined"
                        :rules="[intRange(30,160,'請輸入30–160')]"
                      />
                    </v-col>
                  </v-row>
                  ...
                  <template v-for="symptom in symptomGroups" :key="symptom.key">
                    ...
                    <v-expand-transition>
                      <div v-if="user[symptom.key] === 'yes'">
                        ...
                        <!-- 其他（請自行描述） -->
                        <template v-if="symptom.key==='otherSymptom'">
                          <v-text-field
                        ...
                        <!-- 其餘群組通用欄位 -->
                        <template v-if="!['hypertensionPoorControl','otherSymptom'].includes(symptom.key)">
                          <v-row>
                            <v-col cols="12" sm="6">
                              <v-text-field
                  ...
                  <!-- 病史 -->
                  <v-autocomplete v-model="user.tags1" :items="sickHistory1" label="過去病史1" chips multiple variant="outlined" />
                  <v-autocomplete v-model="user.tags2" :items="sickHistory2" label="過去病史2" chips multiple variant="outlined" />
                  <v-autocomplete v-model="user.FamilyHistory" :items="FamilyHistory" label="家族病史" chips multiple variant="outlined" />
...
const dizzinessOptions = ['又暈又眩', '頭重重的', '合併昏倒/快昏倒/失去意識']
...
const user = ref<any>({
  ...
  palpitationType: [], dizzinessType: [], edemaType: [],
  chestPainLocation: '',
  hypertensionRecentSys: null, hypertensionRecentDia: null,
  hypertensionMedicationAdherence: null, hypertensionMedNames: '',
  otherDescription: '',
...
const FamilyHistory = ['中風','冠狀動脈疾病','猝死']

const oneDecimal = (v:any)=>v===''||v==null||/^(\d+|\d+\.\d)$/.test(String(v))||'僅允許到小數一位'
const positive = (v:any)=>v===''||v==null||Number(v)>0||'需大於0'
...
function submitForm(){console.log('送出資料',user.value);snackbar.value=true}
function toggleTheme(){document.querySelector('html')?.classList.toggle('v-theme--dark')}
</script>
